...
| e2e (app-level) | Playwright, Chromium only | @playwright/test ^1.62.1 | Added 2026-08-12 for Risk #4 only, after re-running the whole risk map through an E2E fit gate: every other top-7 risk is either already covered at a cheaper layer or (the extension) undriveable by Playwright at all, since Firefox MV3 add-ons need `web-ext`/Marionette. Runs `frontend/e2e/*.spec.ts` via `npm run test:e2e` against the **real** app — real router, real `AuthProvider`, real axios interceptors — with two things faked: the session is seeded straight into localStorage (`e2e/support/session.ts`) rather than driven through Cognito's hosted UI, and `/api/*` is stubbed with `page.route()`. Deterministic because the server runs `vite --mode test`, so the app boots the committed `.env.test` placeholders and the OIDC storage key is a constant. No backend, no Neon branch, no credentials, no Anthropic calls. Chromium only: these risks are JS/render wiring, which does not vary by engine. Also covers the print *route* (`/collections/:id/print`) — its auth gate and its fetch→`PrintDocument` handoff — which the harness-based print suite structurally cannot reach, since it never loads the app; checked: 2026-08-12 |
...
| app-level e2e (`npm run test:e2e` in `frontend/`) | local + CI — the "Run app-level E2E tests" step inside the existing `print-tests` job in both `pr-diff.yml` and `deploy.yml` (shipped 2026-08-12) | enforced — it needs no secrets, no database and no browsers beyond the ones that job already installs, so there is nothing to configure. It rides the `print-tests` required status check on the `PR-Needed` ruleset rather than registering a second context; the trade-off is that an E2E failure reports under a job named for the print suite | Two things, both assembly-level. **Risk #4:** a blocked (CORS-shaped) API failure raises the re-auth banner, and a 401 drops the session to the signed-out view — Phase 3's unit tests cover each piece with the others mocked, and these specs are the only thing proving the four boundaries between them (axios interceptor → `connectionIssue` module → `AuthProvider` → render) are wired together. **Risk #2's route half:** that a signed-in user can actually reach a printable document, and that an unauthenticated one cannot — `PrintLayout` carries its own copy of the auth gate. All four specs were break-verified on 2026-08-12: inverting each behaviour turns exactly the matching spec red |
