...
| print browser tests (`npm run test:print`) | CI — its own `print-tests` job in both `pr-diff.yml` and `deploy.yml` (shipped in `testing-print-output-correctness` p5) | `deploy.yml` is auto-gated (`deploy` `needs: [diff, print-tests]`). **The PR path needs a second required-status-check rule** (Settings → Branches) for the `print-tests` job specifically — unlike the frontend/backend rows it is not inside the `diff` job, so the existing rule does not cover it | grey-on-dark printouts under a dark OS theme, a language name overflowing the Language column, and a preview whose page count no longer matches the printed PDF |
