...
// Mirrors extension/src/useSpeech.ts — same no-shared-package reason as
// speech.ts.
...
export function useSpeech (): Speech {
  ...
  useEffect(() => {
    ...
    // started there should not outlive it. The same cleanup stops audio when
    // the web app navigates away from the page.
    return () => {
      cancelled = true
      tokenRef.current += 1
      cancel()
